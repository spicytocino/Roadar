...
if (isset($_POST['advancesearch'])){
	if ($_POST['location'] == ""){
		...
		if ($_POST['year'] == "All"){
			if ($_POST['severity'] == "All"){
				$sql = "SELECT * FROM tbl_incident_report where province like '%" . $_POST['location'] . "%' or city like '%" . $_POST['location'] . "%' or barangay like '%" . $_POST['location'] . "%'  or name_of_road1 like '%" . $_POST['location'] . "%'";
			}else{
				$sql = "SELECT * FROM tbl_incident_report where (province like '%" . $_POST['location'] . "%' or city like '%" . $_POST['location'] . "%' or barangay like '%" . $_POST['location'] . "%'  or name_of_road1 like '%" . $_POST['location'] . "%') and (accident_severity = '" . $_POST['severity'] . "')";
			}
		}else{
			if ($_POST['severity'] == "All"){
				$sql = "SELECT * FROM tbl_incident_report where (province like '%" . $_POST['location'] . "%' or city like '%" . $_POST['location'] . "%' or barangay like '%" . $_POST['location'] . "%'  or name_of_road1 like '%" . $_POST['location'] . "%') and (YEAR(report_date) = '" . $_POST['year'] . "')";
			}else{
				$sql = "SELECT * FROM tbl_incident_report where (province like '%" . $_POST['location'] . "%' or city like '%" . $_POST['location'] . "%' or barangay like '%" . $_POST['location'] . "%'  or name_of_road1 like '%" . $_POST['location'] . "%') and (YEAR(report_date) = '" . $_POST['year'] . "' and accident_severity = '" . $_POST['severity'] . "')";
			}
		}
	}
	if ($_POST['sorting'] == "Sort By Report Status Ascending"){
		$sql = $sql . " order by report_status asc";
	}else{
		$sql =$sql . " order by report_status desc";
	}
	$query =  $sql;
	...
	if(!empty($_POST["go"])) {
		...
		$queryCondition = " and report_no like '%" . $_POST['name'] . "%' or report_date like '%" . $_POST['name'] . "%'";
	}
 $orderby = " ORDER BY CASE report_status
        ...
        ELSE 0
        END DESC, c_no ASC";
	$sql = "select * from tbl_incident_report where reporting_officer_id = " . $_SESSION['myuser_id'] . $queryCondition;
	$href = 'manage_record_officer.php';					
    ...
    <div class="modal-content">
      ...
      <form id="RegisterForm" method="post" >
		...
		<label style="font-weight:bold">Severity</label>
			...
			<label style="font-weight:bold">Accident Causation</label>
			<select class="form-select" name="causation" aria-label="Default select example">
				<option >All</option>
			...
			<select class="form-select" name="sorting" aria-label="Default select example">
				<option >Sort By Report Status Ascending</option>
				<option >Sort By Report Status Descending</option>
			</select>
		</div>
 ...
 <div class="modal fade" id="editform" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <main class="h-100">
		...
		<a class="btn btn-primary" href="incident_report.php" role="button" style="font-size:12px">Report an Incident NOW!</a></br></br>
		<a class="btn btn-danger" href="#" data-toggle="modal" data-target="#searchform" style="font-size:12px">Advance Search</a></br></br>
		<form name="frmlogs" method="post" action="manage_record_officer.php" >
		<div class="input-group mb-3">
		  ...
		  <div class="input-group-append">
			...
			<input type="reset" name="go" class="btn btn-outline-secondary" value="Reset" onclick="window.location='manage_record_officer.php'" >
		  </div>
		</div>
						...
						<div class="col align-self-center ps-0">
							...
							<div class="row">
								...
								<div class="col-auto align-self-center text-end">
									<a href="view_incident_summary.php?id=<?php echo  $row['report_no'] ?>" class="view" title="View" target="blank_" data-toggle="tooltip"><i class="material-icons">&#xE417;</i></a>
									<a href="#" class="edit" title="Edit" onclick="myFunction('<?php echo  $row['report_no'] ?>')"><i class="material-icons">&#xE254;</i></a>
