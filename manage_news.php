...
if( isset($_POST['addform']) ) {
			...
			$sql = "SELECT * FROM tbl_news where title = '" . $title . "'";
			
			$result = $conn->query($sql);
			$count = mysqli_num_rows($result);
			
			if ($count >= 1){
					echo "<script>alert('News Already Exist!');		</script>";
			}else{
				$target_dir = "pictures/";
				...
				echo "Sorry, your file was not uploaded.";
					...
					if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
						$query = "INSERT INTO tbl_news(title,details,post_date,picture) VALUES('$title','$details','$post_date','$newfile')";
						$res = mysqli_query($conn,$query);
						if ($res) {
							echo "<script>alert('News Added to Database');		</script>";
						} else {
			...
			$sql = "SELECT * FROM tbl_news where title = '" . $title . "' and c_no <> " . $update_id ;
			...
			if ($count >= 1){
				...
				echo "Sorry, your file was not uploaded.";
					...
					if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
						$query = "update tbl_news set title = '$title',details = '$details',post_date = '$post_date',picture = '$newfile' where c_no ="  . $_POST['c_no'] ;
...
if(!empty($_POST["go"])) {
	...
	$queryCondition = " where title like '%" . $_POST['name'] . "%' ";
}
...
<div class="modal fade" id="addform" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
 ...
 <div class="modal fade" id="editform" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header border-bottom-0">
        <h5 class="modal-title" id="exampleModalLabel">Modify News</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="form-group">
			<label for="exampleFormControlTextarea1">Title</label>
			<textarea class="form-control" id="title2" name="title" rows="3"></textarea>
		  </div>
		    <div class="form-group">
			<label for="exampleFormControlTextarea1">Details</label>
			<textarea class="form-control" id="details2" name="details" rows="10"></textarea>
		  </div>
		   <div class="form-group">
		    <label for="schoolyear">Picture</label>
            <input type="file" name="fileToUpload" id="fileToUpload">
  ...
  <!--
        ...
        <header class="header position-fixed">
            <div class="row">
                ...
                <div class="mylogo">
					...
					<h1 class="titleheader"> Manage News </h1>
                </div>
   ...
   Add News
		...
		<form name="frmlogs" method="post" action="manage_news.php" >
            <div class="table-title">
                <div class="row">
					...
					<input type="reset" name="go" class="btn btn-default btn-sm w-20 shadow-sm" value="Reset" onclick="window.location='manage_news.php'" ></p>
                </div>
            </div>
            <table class="table table-striped table-hover table-bordered">
				...
				<?php
                    ...
                    <tr>
                        ...
                        <td><?php echo substr($row['title'],0,80);  ?> . .</td>
                        <td><?php echo substr($row['details'],0,100); ?> . .</td>
                        
                        <td>
                            
                            <a href="#" class="edit" title="Edit" data-toggle="modal" data-target="#editform"  data-id="<?php echo  $row['c_no'] ?>"><i class="material-icons">&#xE254;</i></a>
